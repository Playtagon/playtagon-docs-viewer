...
import { loadEnv } from "../scripts/load_env.mjs";
...
function applyEnvOverrides(inputConfig) {
  ...
  if (ignoredFolders) next.ignoredFolders = ignoredFolders;
  return next;
}
...
export async function handleDocsViewerRequest(req, res) {
  const url = new URL(req.url, `http://${req.headers.host || "127.0.0.1"}`);
  ...
  if (config.enabled) {
    ...
    const session = currentSession(req);
    if (!session) {
      if ((req.method === "GET" || req.method === "HEAD") && (url.pathname === "/" || url.pathname.endsWith(".html") || !extname(url.pathname))) {
        redirect(res, "/__auth/login");
      } else {
    ...
    try {
      ...
      send(
        ...
        JSON.stringify({
          ...
          deployment: {
            mode: IS_VERCEL ? "vercel-auth" : "node",
            configWritable: !IS_VERCEL,
            rebuildAvailable: !IS_VERCEL,
          },
        }),
        "application/json; charset=utf-8",
      );
  ...
  if (req.method === "POST" && url.pathname === "/__config") {
    ...
    if (IS_VERCEL) {
      send(
        ...
        JSON.stringify({
          ok: false,
          error: "This Vercel deployment is configured through environment variables. Update Vercel env and redeploy.",
        }),
        "application/json; charset=utf-8",
      );
      return;
  ...
  if (req.method === "POST" && url.pathname === "/__rebuild") {
    ...
    if (IS_VERCEL) {
      send(
        ...
        JSON.stringify({
          ok: false,
          error: "Rebuilds on Vercel are handled by Vercel deployments. Update env or source docs and redeploy.",
        }),
        "application/json; charset=utf-8",
      );
      return;
